...
import { initializeBoard } from "@othello/game-core";
// eslint-disable-next-line @nx/enforce-module-boundaries
import { Board, Player, Coordinate } from '@othello/game-core';
// eslint-disable-next-line @nx/enforce-module-boundaries
import { getValidMoves, applyMove, countScore } from "@othello/game-core";
...
const gameSlice = createSlice({
  ...
  reducers: {
    ...
    makeMove: (state, action: PayloadAction<Coordinate>) => {
      ...
      if(!state.validMoves.some(m => m.row === move.row && m.col === move.col)){
        return;
      }
      ...
      if(state.validMoves.length === 0){
        const backMoves = getValidMoves(newBoard, state.currentPlayer === "B" ? "W" : "B");
